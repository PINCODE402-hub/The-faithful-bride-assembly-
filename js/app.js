...
import { getFirestore, collection, addDoc, doc, setDoc, serverTimestamp, getDocs, query, orderBy, limit } from "https://www.gstatic.com/firebasejs/10.8.0/firebase-firestore.js";
    ...
    try {
        ...
        querySnapshot.forEach((doc) => {
            ...
            div.innerHTML = `
                <h3>${data.title}</h3>
                <p>${data.content}</p>
                ${isAdmin ? `<button class="delete-btn" data-id="${doc.id}" data-collection="announcements">Delete</button>` : ''}
            `;
